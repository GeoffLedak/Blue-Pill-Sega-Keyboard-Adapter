# 1 "C:\\Users\\Turbo9000\\Documents\\Arduino\\blue_pill_keyboard\\blue_pill_keyboard.ino"
# 1 "C:\\Users\\Turbo9000\\Documents\\Arduino\\blue_pill_keyboard\\blue_pill_keyboard.ino"
/*
...
*/
# 57 "C:\\Users\\Turbo9000\\Documents\\Arduino\\blue_pill_keyboard\\blue_pill_keyboard.ino"
// Scancode buffer
...
volatile uint8_t sendBuffer[128];
...
void setup()
{
    ...
    attachInterrupt(PB10, ps2interrupt, FALLING);
    ...
    */
# 123 "C:\\Users\\Turbo9000\\Documents\\Arduino\\blue_pill_keyboard\\blue_pill_keyboard.ino"
...
void loop()
{
    ...
    do{ }
    while( (GPIOA->regs->IDR & 0b0000000100000000 /* PA8*/) != 0b0000000100000000 /* PA8*/ );

    // wait for TH to go low
    do{ }
    while( (GPIOA->regs->IDR & 0b0000000100000000 /* PA8*/) != 0 );
...
short waitForPin(short pin, short value)
{
    ...
    while(numLoops != 0)
    {
        ...
        if( (GPIOA->regs->IDR & 0b0000000100000000 /* PA8*/) != 0 ) // TH went high, transaction aborted
            return 0;

        if( value == 0x0 && (GPIOA->regs->IDR & pin) == value ) // We found the value we want, return 1
            return 1;

        if( value == 0x1 && (GPIOA->regs->IDR & pin) == pin ) // We found the value we want, return 1
            return 1;
...
void Talk_To_Sega()
{
    ...
    if( !waitForPin(0b0000001000000000 /* PA9*/, 0x0) ) { // wait for TR (REQ) to go LOW
        initPins();
        return;
    }
    ...
    if( !waitForPin(0b0000001000000000 /* PA9*/, 0x1) ) { // wait for TR (gen REQ) to go HIGH
        initPins();
        return;
    }
    ...
    else // buffer has stuff in it
    {
        ...
        *index = 0; // put a type byte of 0 to send first
        ...
        while( length > 0 ) // then send the actual scancodes
        {
            ...
            if( !waitForPin(0b0000001000000000 /* PA9*/, 0x0) ) { // wait for TR (gen REQ) to go LOW
                initPins();
                return;
            }
...
void sendNow()
{
    ...
    do { }
    while(PS2busy != 0 && (GPIOB->regs->IDR & 0b0000010000000000) != 0b0000010000000000 );
